...
\centering
\begin{tikzpicture}[thick]
  \draw[layer line] (-2, 0) -- (6.3, 0) node[right,black,font=\footnotesize] {$h=0$ (sumidouro $t$)};
  \foreach \y/\h in {1.5/1, 3.0/2, 6.0/4} {
    \draw[layer line] (-2, \y) -- (6.3, \y) node[right,black,font=\footnotesize] {$h=\h$};
  }
  \draw[red!40,dashed,line width=1.5pt] (-2, 4.5) -- (6.3, 4.5) node[right,red!80!black,font=\footnotesize] {$h=3$ (\textbf{LACUNA})};
  ...
  \draw[->,red,line width=1.5pt] (u) -- node[left,pos=0.5,font=\footnotesize,text=black,align=center,xshift=-5.5mm,fill=white,rounded corners=2pt,inner sep=2pt] {Queda de 2 níveis\\(inadmissível em $A_f$)} (b);
  \draw[->,red,line width=1.5pt] (u) -- node[right,pos=0.5,font=\footnotesize,text=black,align=center,xshift=5.5mm,fill=white,rounded corners=2pt,inner sep=2pt] {$h(u) \not\le h(c) + 1$\\(inadmissível em $A_f$)} (c);
