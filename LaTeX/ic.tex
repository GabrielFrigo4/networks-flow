...
\begin{enumerate}
    ...
    \item \textbf{T-links (Terminal Links):} Para cada pixel $p_i$, criam-se dois arcos conectando-o aos terminais:
    \begin{itemize}
        \item $(s, p_i)$ com peso $w_s(p_i)$: afinidade com o modelo de cor do \textit{foreground}.
        \item $(p_i, t)$ com peso $w_t(p_i)$: afinidade com o modelo de cor do \textit{background}.
    \end{itemize}
    Pixels marcados explicitamente pelo usuário como \textit{foreground} recebem $w_s = \infty$ e $w_t = 0$, e vice-versa para o \textit{background}.
\end{enumerate}
