...
A versão otimizada (Código~\ref{lst:push_relabel_improved}, Apêndice~\ref{ap:push_relabel_improved}) implementa a política \textit{Highest Label}: em cada iteração, o vértice ativo com a \textbf{maior altura} é selecionado para processamento. A estrutura de dados empregada é um vetor de \textit{buckets} indexados por altura (\lstinline{active_buckets}), com um ponteiro \lstinline{highest_active} que rastreia o nível mais alto ocupado. Esta política de seleção, combinada com a Heurística de Lacuna, reduz a complexidade para $\mathcal{O}(|V|^2 \sqrt{|A|})$ (veja~\cite{goldberg1988new}).
