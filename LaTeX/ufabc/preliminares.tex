...
\subsection{Estrutura do Documento e Guia de Leitura}\label{sec:leitura}

A organização do relatório estrutura-se em:

\begin{enumerate}[leftmargin=1.5em, itemsep=0.15em, topsep=0.2em, parsep=0em]
    \item \textbf{Corpo Principal (Seções~\ref{sec:introducao} a~\ref{sec:conclusoes}, páginas~\pageref{sec:introducao} a~\pageref{sec:conclusoes}):} Apresenta a fundamentação teórica, demonstrações dos teoremas estruturais, descrição dos algoritmos de fluxo máximo e custo mínimo, reduções combinatórias, segmentação de imagens e metodologia experimental.
\ifapendicecodigo
    \item \textbf{Apêndices Computacionais (Apêndices~\ref{ap:flownetwork} a~\ref{ap:github}, páginas~\pageref{ap:flownetwork} a~\pageref{ap:github}):} Documentam o código-fonte integral desenvolvido em C++23.
\else
    \item \textbf{Repositório Aberto e Apêndices de Código:} O código-fonte integral em C++23, instâncias DIMACS e scripts de teste encontram-se documentados na versão estendida deste relatório, na monografia integral (\texttt{book.pdf}) e no repositório GitHub do projeto.
\fi
\end{enumerate}

\noindent \textbf{Roteiro de Leitura Rápida:}
Para uma leitura focada nas contribuições centrais, recomenda-se a consulta aos seguintes tópicos:
\begin{itemize}[leftmargin=1.5em, itemsep=0.15em, topsep=0.2em, parsep=0em]
    \item \textbf{Subseção~\ref{sec:push_relabel_gap} (página~\pageref{sec:push_relabel_gap}):} Heurística de lacuna (\textit{Gap Heuristic}) no algoritmo Push-Relabel;
    \item \textbf{Subseção~\ref{sec:segmentacao} (página~\pageref{sec:segmentacao}):} Segmentação Interativa de Imagens via corte mínimo em grafos;
    \item \textbf{Subseção~\ref{sec:ssp_dijkstra} (página~\pageref{sec:ssp_dijkstra}):} \textit{Successive Shortest Path} com potenciais nodais ($\bar{w}_\pi \ge 0$) e Dijkstra;
    \item \textbf{Subseção~\ref{sec:network_simplex} (página~\pageref{sec:network_simplex}):} Total Unimodularidade e algoritmo \textit{Network Simplex};
    \item \textbf{Subseção~\ref{sec:benchmarks} (página~\pageref{sec:benchmarks}):} Protocolo experimental e resultados em instâncias DIMACS;
    \item \textbf{Seção~\ref{sec:conclusoes} (página~\pageref{sec:conclusoes}):} Síntese dos resultados e perspectivas futuras.
\end{itemize}

\section{Fundamentação teórica}\label{sec:fundamentacao}
O problema do $st$-fluxo máximo, onde se busca maximizar a quantidade de fluxo enviada de uma fonte $s$ para um destino $t$, é um dos problemas centrais deste estudo (veja~\cite{ford1956maximal}). Foram analisados os algoritmos de Ford-Fulkerson, Edmonds-Karp, Dinic e Push-Relabel (Goldberg-Tarjan) (veja~\cite{ford1956maximal, edmonds1972theoretical, dinic1970algorithm, goldberg1988new}).
O projeto também contempla o problema de fluxo de custo mínimo. Para este fim, foram desenvolvidos e analisados os algoritmos Cycle Canceling, Successive Shortest Path (com SPFA e Dijkstra acelerado por potenciais) e \textit{Network Simplex} (veja~\cite{dantzig1951application, ahuja1993network}).
Diversos problemas combinatórios, como o emparelhamento máximo em grafos bipartidos, são resolvidos via redução para fluxo (veja~\cite{ahuja1993network}).
